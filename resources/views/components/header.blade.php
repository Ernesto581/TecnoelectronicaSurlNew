...
@php
    $heroPages = ['/', 'quienes-somos', 'servicio-domicilio'];
    $isHero = in_array(request()->path(), $heroPages);
    $isScrolled = $solid || !$isHero;
@endphp

<header
    x-data="{ mobileOpen: false, scrolled: {{ $isScrolled ? 'true' : 'false' }}, cartCount: 0 }"
...
>
    <div class="h-full px-4 md:px-8 max-w-[1600px] mx-auto flex items-center justify-between">
        ...
        <div class="hidden md:flex items-center gap-4">
            <button :class="scrolled ? 'text-gray-600 hover:text-[#46A040] hover:bg-gray-100' : 'text-white/80 hover:text-white hover:bg-white/10'" class="p-2 transition-colors rounded-full">
            ...
            <a href="/login" :class="scrolled ? 'text-[#46A040] bg-[#ecf8ef] hover:bg-[#d2efdc]' : 'text-white/90 bg-white/10 hover:bg-white/20'" class="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-full transition-all">
        ...
        <div class="flex xl:hidden items-center gap-4 z-50">
        ...
        <div x-show="mobileOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" class="absolute top-full left-0 w-full bg-white shadow-xl xl:hidden border-b border-gray-200 flex flex-col" style="display: none;">
            <div class="p-4 border-b border-gray-100 bg-gray-50 flex items-center">
                <x-icon name="search" class="w-5 h-5 text-gray-400 mr-2" />
                <input type="text" placeholder="Buscar productos..." class="bg-transparent border-none outline-none w-full text-sm text-gray-800 focus:ring-0" />
            </div>
