...
$link = $link ?: '/tienda/producto/' . $product->slug;
@endphp
...
<div class="group bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col h-full">
    ...
    <div class="p-5 flex flex-col flex-grow">
        <p class="text-[10px] text-[#46A040] font-black uppercase tracking-[0.2em] mb-1">{{ $product->category->name ?? 'General' }}</p>
        <h3 class="text-lg font-bold text-gray-900 mb-2 leading-tight group-hover:text-[#46A040] transition-colors">{{ $product->name }}</h3>
        ...
        <div class="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between">
            ...
            <a href="{{ $link }}" class="bg-gray-900 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-[#46A040] transition-all active:scale-95 shadow-md">Detalles</a>
        </div>
    </div>
</div>
